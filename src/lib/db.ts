...
export async function createIdea(input: Partial<Idea>): Promise<Idea> {
  ...
  const i: Idea = {
    ...
    photo_data: input.photo_data ?? null,
    voice_data: input.voice_data ?? null,
    voice_duration_secs: input.voice_duration_secs ?? null,
    archived: false,
    deleted: false,
...
export async function createRememberItem(input: Partial<RememberItem>): Promise<RememberItem> {
  ...
  const r: RememberItem = {
    ...
    favorite: input.favorite ?? false,
    deleted: false,
    reminder_freq: input.reminder_freq ?? null,
